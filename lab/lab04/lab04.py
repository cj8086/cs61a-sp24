    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1, amount // price + 1):
                add(fruits[1:], amount - price * k, cart + f"[{k} {fruit[:-1] if k == 1 else fruit}]")
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    dis_a_sq = (get_lat(city_a) - lat) ** 2 + (get_lon(city_a) - lon) ** 2
    dis_b_sq = (get_lat(city_b) - lat) ** 2 + (get_lon(city_b) - lon) ** 2
    if dis_a_sq > dis_b_sq:
